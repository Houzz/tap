...
struct Token : ScannerToken {
  ExpressionPtr exp;
  StatementPtr stmt;
  // TODO(liang): TypeAnnotationPtr typeAnnotation;
  Tap::TapType* tapType;
  ...
  Token& operator=(const Token& other) {
    ...
    exp = other.exp;
    stmt = other.stmt;
    // TODO(liang): typeAnnotation = other.typeAnnotation;
    // tapType = other.tapType;
    return *this;
  }
  ...
  void reset() {
    exp.reset();
    stmt.reset();
    // TODO(liang): typeAnnotation.reset();
    tapType = nullptr;
    ScannerToken::reset();
  }

  const std::string typeAnnotationName() {
    // TODO(liang): return (typeAnnotation) ? typeAnnotation->fullName() : "";
    return tapType ? tapType->toString() : "unknown";
  }
};
...
struct Parser : ParserBase {
    /* TODO(liang):
  static StatementListPtr ParseString(const std::string& input, AnalysisResultPtr ar,
  ...
  */
public:
  Parser(Scanner &scanner, const char *fileName, int fileSize = 0);
  ...
  virtual void parseFatal(const Location* loc, const char* msg) {
      printf("parseFatal parse error: %s\n", msg);
  }
  ...
  void onUnaryOpExp(Token &out, Token &operand, int op, bool front, std::string docComment = "");
  ...
  // TODO(liang): FileScopePtr getFileScope() { return m_file; }
...
private:
  ...
  // TODO(liang): FileScopePtr m_file;
  ...
  // TODO(liang): std::vector<ScalarExpressionPtr> m_compilerHaltOffsetVec;
  ...
  // TODO(liang): std::vector<StringToExpressionPtrVecMap> m_staticVars;
  bool m_lambdaMode;
  ...
  // TODO(liang): void setHasNonEmptyReturn(ConstructPtr blame);
  ...
  */
  ...
  // TODO(liang): void checkThisContext(ExpressionListPtr params, ThisContextError error);
...
public:
  ...
  private:
    ...
    // TODO(liang): hphp_string_imap<NameEntry> m_aliases;
...
private:
  ...
  // TODO(liang): AliasTable m_nsAliasTable;
...
inline void HPHP_PARSER_ERROR(const char* fmt,
                       Parser* p,
                       Args&&... args) {
  throw; // TODO(liang):  ParseTimeFatalException(p->file(), p->line1(), fmt, args...);
}

inline void HPHP_PARSER_ERROR(const char* msg, Parser* p) {
  throw; // TODO(liang):  ParseTimeFatalException(p->file(), p->line1(), "%s", msg);
}
