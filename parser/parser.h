...
struct ParserBase {
   ...
   */
  // TODO(liang): static bool IsAnonymousClassName(folly::StringPiece name);
...
public:
  ...
  int scan(ScannerToken *token, Location *loc) {
    int res = m_scanner.getNextToken(*token, *loc);
    return res;
  }
...
protected:
  ...
  // TODO(liang): hphp_string_imap<std::string> m_aliases;
  std::unordered_map<std::string, std::string> m_aliases;
  // TODO(liang): hphp_string_imap<int> m_seenAnonClasses;
  std::unordered_map<std::string, int> m_seenAnonClasses;
};
