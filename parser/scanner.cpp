...
void ScannerToken::xhpDecode() {
  ...
  char *ret = NULL; // TODO(liang): commented out previous line
...
Scanner::Scanner(const std::string& filename, int type, bool md5 /* = false */)
  ...
  if (m_stream->fail()) {
    ...
    // TODO(liang): throw FileOpenException(m_filename);
    throw;
  }
#endif
  if (md5) computeMd5();
...
void Scanner::computeMd5() {
  ...
  // TODO(liang): m_md5 = string_md5(folly::StringPiece{ptr, length});
  free(ptr);
}
...
void Scanner::error(const char* fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
  // TODO(liang): string_vsnprintf(m_error, fmt, ap);
  va_end(ap);
}
...
void Scanner::warn(const char* fmt, ...) {
  ...
  va_start(ap, fmt);
  std::string msg;
  // TODO(liang): string_vsnprintf(msg, fmt, ap);
  va_end(ap);
        ...
        if (++i < len) {
          switch (str[i]) {
            ...
            case 'u': {
              ...
              */
              if (!valid) {
                throw; // TODO(liang): fatal("Invalid UTF-8 codepoint escape sequence");
              }
